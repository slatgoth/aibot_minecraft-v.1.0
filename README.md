...
## Последнее обновление (v1.2.6)
- Исправлена утечка памяти при переподключениях (бот не будет раздуваться до OOM).
- Бот открывает двери и умеет прыгать по команде.
- Улучшено использование верстака и печки.
